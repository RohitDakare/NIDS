...
class SecureMongoManager:
    ...
    def create_indexes(self):
        ...
        try:
            ...
            alerts_collection.create_index([("dest_ip", 1)])
            alerts_collection.create_index([("protocol", 1)])
    ...
    def insert_packet(self, packet_data: Dict[str, Any]) -> bool:
        """Securely insert raw packet data into MongoDB"""
        try:
            if self.db is None:
                return False
            
            # Limited sanitization for raw packets
            self.db.packets.insert_one(packet_data)
            return True
        except Exception as e:
            logger.error(f"Failed to insert packet into MongoDB: {e}")
            return False
