...
class NIDSOrchestrator:
    ...
    def _process_packet(self, packet: PacketInfo):
        ...
        try:
            ...
            try:
                if secure_mongo.db is not None:
                    # Use model_dump() for Pydantic v2, dict() for v1
                    packet_data = packet.model_dump() if hasattr(packet, 'model_dump') else packet.dict()
                    secure_mongo.insert_packet(packet_data)
            except Exception as db_err:
                logger.debug(f"Failed to persist packet to DB: {db_err}")
