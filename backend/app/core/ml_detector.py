...
class MLDetector:
    ...
    def __init__(self, config: MLModelConfig):
        ...
        if self.config.model_path and os.path.exists(self.config.model_path):
            # We don't call load_model(cls) here to avoid recursion.
            # Instead, we perform the load logic directly or call a helper.
            self._load_model_from_path(self.config.model_path)
        else:
            logger.info("No pre-trained model found. System will need training before ML detection is active.")
    ...
    def _init_preprocessing_pipeline(self):
        ...
        # DO NOT set is_loaded here. It should only be True when actually trained or loaded from disk.
        # self.is_loaded = True
    ...
    def _load_model_from_path(self, model_path: str) -> bool:
        """Internal helper to load model from path without recursion."""
        try:
            # Verify model integrity
            if not model_security.verify_model_integrity(model_path):
                logger.warning(f"Model integrity check failed for {model_path}. Proceeding with caution.")
            ...
            # Update our pipeline and model
            if isinstance(model, (Pipeline, ImbPipeline)):
                self.pipeline = model
                # Extract classifier from pipeline
                if hasattr(model, 'named_steps'):
                    self.model = model.named_steps.get('classifier') or list(model.named_steps.values())[-1]
                else:
                    self.model = model.steps[-1][1]
                logger.info("Successfully loaded fitted pipeline")
            else:
                self.model = model
                # Reconstruct pipeline if we only got the model
                self._init_preprocessing_pipeline()
                if hasattr(self.pipeline, 'named_steps'):
                    self.pipeline.named_steps['classifier'] = self.model
                else:
                    self.pipeline.steps[-1] = ('classifier', self.model)
                logger.info("Successfully loaded model and reconstructed pipeline")

            # Final check - verify if it's fitted
            try:
                from sklearn.utils.validation import check_is_fitted
                check_is_fitted(self.pipeline)
                self.is_loaded = True
                logger.info(f"ML Model fully loaded and ready: {model_path}")
                return True
            except Exception as e:
                logger.error(f"Loaded model is not fitted: {e}")
                # Try dummy fitting as a last resort if it's not fitted
    ...
    def _fit_with_dummy_data(self):
        """Fit the pipeline with dummy data to satisfy sklearn's check_is_fitted."""
        try:
            logger.info("Fitting pipeline with dummy data...")
            all_cols = self.numerical_columns + self.categorical_columns
            dummy_data = []
            for _ in range(10):
                row = {}
                for col in all_cols:
            ...
            logger.info("Dummy fitting successful")
        except Exception as e:
            logger.error(f"Dummy fitting failed: {e}")
